...
namespace Renderer {
	Sprite::Sprite(const std::shared_ptr<Renderer::Texture2D> pTexture,
		const std::shared_ptr<Renderer::ShaderProgram> pShaderProgram,
		const glm::vec2& position,
		const glm::vec2& size,
		const float rotation):
		
		m_pTexture(std::move(pTexture)), // чо за std::move 
		...
		m_rotation(rotation)
	{
		...
		glBindVertexArray(m_vao); // а если я сделаю наоборот?
