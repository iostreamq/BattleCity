...
Level::Level(const std::vector<std::string>& levelDescription)
{
    ...
    m_enemyRespawn_2 = { BLOCK_SIZE  * ((m_width + 1) / 2), BLOCK_SIZE * m_height - BLOCK_SIZE / 2 };
    ...
    //border top
    m_levelObjects.emplace_back(std::make_shared<Border>(glm::vec2(BLOCK_SIZE, (m_height*16)+(BLOCK_SIZE/2)), glm::vec2(m_width * BLOCK_SIZE, BLOCK_SIZE / 2.f), 0.f, 0.f));/////////////////
    //border left
    m_levelObjects.emplace_back(std::make_shared<Border>(glm::vec2(0.f, 0.f), glm::vec2(BLOCK_SIZE, BLOCK_SIZE * (m_height+2)), 0.f, 0.f));
    //border right
    m_levelObjects.emplace_back(std::make_shared<Border>(glm::vec2((1+m_width)*BLOCK_SIZE, 0.f), glm::vec2(2.f * BLOCK_SIZE, BLOCK_SIZE * (m_height + 2)), 0.f, 0.f));
